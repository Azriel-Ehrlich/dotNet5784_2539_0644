...
internal class TaskImplementation : BlApi.ITask
{
    ...
    void RecursionCheckCyclicDependency(List<int> taskDependencies, int root)
    {
        if (taskDependencies is null) return;
        foreach (var dep in taskDependencies) // search for the root in the dependencies. If found it's a cyclic dependency
        {
            if (dep == root)
                throw new BO.BlCannotUpdateException("Circular dependency");
            RecursionCheckCyclicDependency((Read(dep).Dependencies ?? new()).Select(t => t.Id).ToList(), root);
        }
    }
    ...
    {
        RecursionCheckCyclicDependency((task.Dependencies ?? new()).Select(t => t.Id).ToList(), task.Id);
    }
